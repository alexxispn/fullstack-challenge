...
This branch implements the improvements identified in "What I Would Improve" above. Each section below follows a problem → decision → trade-off structure; implementation details are in the code and commit history.

#### Rich domain model with Value Objects

Replaced anemic `Product`/`CreateProduct` interfaces with a `Product` aggregate root class + 4 Value Objects (`Price`, `Stock`, `Category`, `ProductName`) that enforce invariants at construction. A `DomainError` hierarchy (`InvalidPriceError`, `InvalidStockError`, etc.) maps to 400 via a global exception filter. DTO validation is kept as defense in depth at the HTTP boundary. Trade-off: `fromPersistence()` re-validates through VOs on every database read — acceptable at this scale, would add a `fromTrustedSource()` path for high-throughput reads.

#### Testcontainers integration tests

The fake repository simulated filtering in JavaScript but never verified real SQL (`LOWER()`, `NUMERIC` coercion, `RETURNING`, `ORDER BY` on timestamps). Added 7 integration tests against a real PostgreSQL 16 container via Testcontainers. Refactored `DatabaseService` to accept an injected `Pool` instead of creating its own from env vars — tests inject the container's pool directly, no `@Optional()` hacks. Trade-off: ~4s + container startup vs instant, but separated via `jest.integration.config.ts` so `npm test` stays fast.

#### Test doubles refactor (ISP + hand-crafted doubles)

The controller tests used a `FakeProductsRepository` that replicated filtering logic — a "smart double" anti-pattern with no contract tests. Split `ProductRepositoryPort` into `ProductReader` and `ProductWriter` (Interface Segregation), then replaced the fake with inert doubles: `ProductsInCatalog` (stub), `ProductCatalogSpy` (spy), `ListProductsCriteriaSpy` (wiring spy). Controller tests now verify DTO validation and parameter wiring via spies; filtering correctness lives in the integration tests. Trade-off: more test files, but each double is trivial and has a single responsibility.

#### Object Mothers and shared test infrastructure

Extracted duplicated test data into `ProductExamples` (persisted products) and `CreateProductExamples` (commands) as Object Mothers with semantic names (`aRing()`, `aNecklace()`). Shared test doubles extracted to `test/test-doubles/`. Trade-off: one level of indirection, but a single place to update when the domain model changes.

#### Code smell remediation

Systematic analysis against the Jerzyk catalog (56 smells). After filtering false positives (~40 candidates → 7 real findings), applied 4 targeted fixes: declarative filter array replacing imperative query builder, `@TransformBoolean()` decorator, `withTransaction()` helper for migrations, and `CENTS_PER_UNIT` named constant.

#### Vertical Slice Architecture

Reorganized from horizontal layers (`domain/`, `application/`, `adapters/`, `infrastructure/`, `ports/`) to vertical feature slices. The first level of folders now describes what the application does, not which architecture pattern it uses:
...
```
npm test                    → 32 unit tests      (1.2s, no Docker)
